...
export const fetchAwardComparisonSnapshot = (aidYear, effectiveDate) => {
  return (dispatch, getState) => {
    const { awardComparisonSnapshot } = getState();

    // if (awardComparisonSnapshot.loaded || awardComparisonSnapshot.isLoading) {
    //   return new Promise((resolve, _reject) =>
    //     resolve(awardComparisonSnapshot)
    //   );
    // } else {
    dispatch(fetchAwardComparisonSnapshotStart(aidYear, effectiveDate));

    return axios
      .get(`/api/my/financial_aid/award_comparison/${aidYear}/${effectiveDate}`)
      .then(response => {
        dispatch(
      ...
      .catch(error => {
        if (error.response) {
          ...
          dispatch(
            fetchAwardComparisonSnapshotFailure(aidYear, effectiveDate, failure)
          );
        }
      });
    // }
